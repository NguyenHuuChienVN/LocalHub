	...
	return (
		<div className="mx-10 py-8 max-sm:mx-4 max-sm:py-5">
			...
			<div className="mt-6 grid gap-8 lg:grid-cols-[minmax(0,1.2fr)_minmax(300px,0.8fr)]">
				...
				<aside className="h-fit rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-sm text-slate-500">Giá từ</p><strong className="mt-1 block text-2xl font-black text-slate-950">{formatPrice(service.price)}</strong><p className="mt-2 text-xs text-slate-500">Mức giá có thể thay đổi theo yêu cầu thực tế.</p><Link className="mt-6 block w-full rounded-lg bg-blue-600 px-4 py-3 text-center text-sm font-bold text-white hover:bg-blue-700" state={getAuthUser() ? undefined : { from: bookingPath }} to={getAuthUser() ? bookingPath : "/login"}>Đặt lịch ngay</Link><Link className="mt-3 block text-center text-sm font-bold text-blue-600" to="/support">Cần tư vấn trước khi đặt?</Link></aside>
			</div>
		</div>
	);
}
